...
colours = c("#FFFFFF","#F6BDC0", "#F1959B", "#F07470", "#EA4C46", "#DC1C13")

household_data2 %>%
  mutate(county_name = gsub(pattern = "County", replacement = "", household_data2$county_name)) %>%
  ...
  coord_sf(datum = NA, crs = 5070) +
  # coord_sf(crs = 102003) +
  scale_fill_gradientn(colors = colours, labels = scales::dollar,
                       guide_colorbar(title = "Median Household Income",
                                      title.position = "top",
                                      barwidth = 0.5,
                                      barheight = 4)) +
  ...
  theme_minimal() +
  theme(legend.position = "top") +
  geom_sf_text(aes(label = medhhincome, geometry = geometry), color = "black", fontface = "bold", size = 4) +
  # geom_sf_text(aes(label = county_name, geometry = geometry, vjust = -1), size = 5)
  geom_sf_label((aes(label = county_name, geometry = geometry, vjust = -0.5))) +
  theme(axis.title = element_blank(),
        axis.ticks = element_blank(),
        axis.text = element_blank(),
        panel.grid = element_blank(),
